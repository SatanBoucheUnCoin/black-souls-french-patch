...
class RGSSArchive:
    ...
    def create(cls, location, version):
        ...
        import random
        magic = random.randint(1, 0xFFFFFFFF) if version == 3 else 0xDEADCAFE
    ...
    def write_entries_rgss3a(self, root):
        ...
        for entry in self.entries:
            off += 16 + len(entry.name.replace('/', '\\').encode('utf-8'))
        off += 4
        ...
        for entry in self.entries:
            ...
            off += entry.data.size
            entry.data.magic = random.randint(0, 0x7FFF)  # Small random key like original
        ...
        for entry in self.entries:
            ...
            name_bytes = entry.name.replace('/', '\\').encode('utf-8')
            wu32(self.stream, len(name_bytes) ^ self.magic)

            encrypted_name = bytearray(name_bytes)
            for i in range(len(encrypted_name)):
